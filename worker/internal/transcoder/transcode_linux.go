...
func resolveHWArgs(encoder config.Encoder, swDecode bool, outW, outH int) (hwArgs []string, vfFilter string) {
	...
	case hardware.EncoderQSV:
		if !swDecode {
			...
			// DeviceIndex stores the integer suffix of /dev/dri/renderD<N>. The
			// pre-computed outW/outH preserve aspect, so vpp_qsv's lack of pad
			// support is no longer a problem — nothing pads.
			hwArgs = []string{
