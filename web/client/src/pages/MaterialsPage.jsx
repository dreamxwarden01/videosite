...
export default function MaterialsPage() {
  ...
  const fetchCourses = useCallback(async () => {
    try {
      const { data, ok } = await apiGet('/api/materials/courses');
      if (ok && data?.courses) {
  ...
  return (
    <div>
      <div className="flex-between mb-3">
        ...
        {user.permissions.uploadAttachments && (
            ...
            <tbody>
              {paginatedCourses.map(c => (
                ...
                >
                  ...
                  <td>{c.last_material_at ? new Date(c.last_material_at).toLocaleDateString() : '-'}</td>
                </tr>
              ))}
              {courses.length === 0 && (
                <tr>
