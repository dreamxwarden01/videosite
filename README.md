...
- **Whole-file IDR scan.** Every source's keyframe layout is read in full before any decision is made, from the MP4 `moov` sample index (`stss`/`stts`/`ctts`/`elst`) rather than by walking packets — 2 KB–4 MB of I/O and tens of milliseconds on a multi-hour lecture, against seconds for a packet scan. Containers without a usable index (MKV, MPEG-TS, fragmented MP4) fall back to `ffprobe` automatically. Scanning the whole file matters: a source can measure perfectly constant over its first two minutes and still carry irregular GOPs later, and a decision made on the head of the file is a decision made on the wrong data.
- **Segmentation is decided in frames, not seconds.** Each rendition resolves a `SegmentPlan` — GOP in frames, segment as a whole number of GOPs — on its own effective output rate, and FFmpeg is driven from those integers (`-g`, `-force_key_frames expr:eq(mod(n,G),0)`, and an `-hls_time` converted back through the exact frame-rate rational). FFmpeg's HLS muxer cuts on an *absolute* accumulated threshold, so any per-segment mismatch between the requested and produced length compounds until it costs a whole GOP; naming the grid in frames removes the mismatch instead of shrinking it. Remux is the one path that keeps a safety margin, sized as a fraction of the GOP, because a stream copy inherits keyframe timestamps it does not control.
- **Remux is chosen by simulating it, not by a heuristic.** A stream copy can only cut where the source already has an IDR, so the playlist it would produce is fully determined by the keyframe layout already read during probing. The worker replays FFmpeg's cut rule over that layout and keeps the bit-exact copy only when the predicted segments pass the same uniformity test the DASH writer applies; otherwise that rendition is re-encoded onto the worker's own grid. This is deliberately not a tolerance on inter-IDR deviation — two 59.94 fps lectures measured 0.0339 s and 0.0317 s maximum deviation, indistinguishable to any threshold, yet one remuxes uniformly and the other cannot, because what matters is how *often* a source runs long (0.35% of GOPs versus 7%), not by how much.
- **Hardware encoders are pinned to real IDRs.** NVENC and QSV both answer a forced-keyframe request with a plain I-frame unless told otherwise, and a non-IDR I-frame carries no keyframe flag for the muxer to split on — so `-forced-idr` (NVENC) / `-forced_idr` (QSV) are set alongside the keyframe expression. Without them a QSV rendition segments on whatever sparse IDRs the encoder emits on its own.
- **Audio keeps the source sample rate**, snapped only if AAC-LC cannot represent it, and its segment length is an exact whole number of 1024-sample AAC frames. Resampling is what makes fragment durations wobble; not resampling is what keeps a 44.1 kHz lecture on one segment length for the whole file.
- DASH manifest collapses from `<SegmentTimeline>` (one entry per segment per rendition) to per-Representation `<SegmentTemplate duration="…">` when segments are uniform, scaling O(profiles) instead of O(segments × profiles); μs-precision timescale eliminates declared-vs-actual drift on multi-hour content. Video and audio AdaptationSets decide uniformity independently.
